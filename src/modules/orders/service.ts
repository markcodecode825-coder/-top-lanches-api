...
function isServiceModeCode(
  value: string
): value is ServiceModeCode {
  return (
    value === 'delivery' ||
    value === 'pickup' ||
    value === 'dine_in'
  ...
  return {
    ...
    cashChangeForFormatted:
      order.cashChangeForInCents === null
        ? null
        : formatCents(order.cashChangeForInCents),
    status: order.status,
    notes: order.notes,
    ...
    items: order.items.map((item) => ({
      ...
      unitPriceFormatted: formatCents(
...
export async function createOrder(
  ...
  const rawServiceMode = input.serviceMode;
  const rawPaymentMethod = input.paymentMethod;

  if (!isServiceModeCode(rawServiceMode)) {
    throw new AppError(
      'INVALID_SERVICE_MODE',
      422,
      'Modalidade de atendimento inválida'
    );
  }

  if (!isPaymentMethodCode(rawPaymentMethod)) {
    throw new AppError(
  ...
  const serviceModeCode: ServiceModeCode =
    rawServiceMode;
  const paymentMethodCode: PaymentMethodCode =
    rawPaymentMethod;
  ...
  const [
    ...
    prisma.serviceMode.findUnique({
      where: { code: serviceModeCode }
    }),
  ...
  if (!serviceMode?.enabled) {
    throw new AppError(
      'INVALID_SERVICE_MODE',
      422,
      'Modalidade de atendimento indisponível'
    );
  }
  ...
  if (!status.isOpen && !acceptWhenClosed) {
    ...
    throw new AppError(
      'BUSINESS_CLOSED',
      409,
      `${business.name} está fechada no momento.`
    );
  }
  ...
  for (const quantity of quantities.values()) {
    if (quantity > 99) {
      throw new AppError(
        'VALIDATION_ERROR',
        422,
        'Quantidade máxima por produto é 99'
      );
    }
  }
  ...
  const snapshots = productIds.map((productId) => {
    ...
    const unitPriceInCents =
      effectivePriceInCents(product);
    const subtotalInCents =
      unitPriceInCents * quantity;

    if (
      !Number.isSafeInteger(subtotalInCents) ||
      subtotalInCents > MAX_MONEY_IN_CENTS
    ) {
      throw new AppError(
        'VALIDATION_ERROR',
        422,
        'Valor do pedido excede o limite suportado'
      );
    }
  ...
  const subtotalInCents = snapshots.reduce(
    (sum, item) =>
      sum + item.subtotalInCents,
    0
  );

  const deliveryFeeInCents: number | null =
    null;
  const totalInCents = subtotalInCents;

  if (
    !Number.isSafeInteger(totalInCents) ||
    totalInCents > MAX_MONEY_IN_CENTS
  ) {
    throw new AppError(
      'VALIDATION_ERROR',
      422,
      'Valor total do pedido excede o limite suportado'
    );
  }

  let cashChangeForInCents: number | null =
    null;
  ...
  if (
    ...
    if (
      typeof input.cashChangeFor !== 'number' &&
      typeof input.cashChangeFor !== 'string'
    ) {
      throw new AppError(
        'VALIDATION_ERROR',
        422,
    ...
    if (
      cashChangeForInCents < totalInCents
    ) {
      throw new AppError(
        'VALIDATION_ERROR',
        422,
        'cashChangeFor deve ser maior ou igual ao total do pedido'
      );
    }
  }
  ...
  try {
    const order = await prisma.$transaction(
      async (tx) => {
        ...
        const orderNumber =
          `TL-${String(counter.value).padStart(
            6,
            '0'
          )}`;
